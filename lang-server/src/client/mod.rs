...
pub trait Client: Send + Sync + 'static {
    fn log_message<M: Display + Send>(
        &self,
        typ: MessageType,
        message: M,
    ) -> impl Future<Output = ()> + Send;
    fn show_message<M: Display + Send>(
        &self,
        typ: MessageType,
        message: M,
    ) -> impl Future<Output = ()> + Send;
...
impl Client for tower_lsp::Client {
    ...
    async fn show_message<M: Display>(&self, typ: MessageType, message: M) {
...
impl Client for () {
    ...
    async fn show_message<M: Display>(&self, _: MessageType, _: M) {}
}
