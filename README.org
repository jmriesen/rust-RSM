...
* Techniques/Concepts 
** Unit testing
    The more unit tests I the more useful I realize unit tests are, and the less they seem to be about double checking my work.
...
*** Concept overview 
   ...
   - First if it allows you to imagine how your code will be called. 
      If writing unit tests is hard or requites convoluted logic for some reason, it is going to be hard and convoluted to write the application code relies on this unit of code.
...
*** How is it used for this project.
    ...
    The bindgen and cbindgen tools do most of the work automating generating type and function definitions.
    However there are a few things project specific things that have to be kept in mind:
    - Don't blindly trust the generated type definitions.
      The C code uses dynamically sized types, however the header files/generated rust types assume these types occupy there max size.
...
*** Concept Overview 
    ...
    A/B testing at its core simply testing the invariant that system A should behave the same as system B.
*** Use in this project
    ...
    The fact that I have the original copy of the code that I can A/B test against is fairly artificial, so I think I will learn by focusing on other forms of unit tests.
