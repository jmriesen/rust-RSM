 ...
 */
short routine_temp(int runtime,u_char **src_ptr,u_char **comp,partab_struct *partab_ptr)                                                      // parse routine ref
{
    ...
    if (runtime == -2) {                                                        // if $TEXT() compile
        if (!isinder && !var_empty(tag)) {                                      // if not indirected & tag is defined
            *(*comp)++ = OPSTR;                                                // string follows
            us = (u_short) i;
            assert(sizeof(us) == sizeof(u_short));
            memcpy((*comp), &us, sizeof(u_short));
            (*comp) += sizeof(u_short);
            for (j = 0; j < i; *(*comp)++ = tag.var_cu[j++]) continue;         // copy tag
        ...
        if (!runtime) {                                                         // if just compiling
            ...
            *(*comp)++ = OPCAT;                                                // concatenate
            ...
            while (TRUE) {                                                      // while we have digits
                ...
                if (!isdigit(*(*src_ptr))) break;

                if (!dp) {                                                      // if part of int
                    offset = (offset * 10) + *(*src_ptr)++ - '0';               // convert it
                } else {
                    (*src_ptr)++;                                               // just ignore it
                }
            }
        ...
        if (c != '@') {                                                         // if not indirect
            ...
            if (isinder) {                                                      // is indirect already
                *(*comp)++ = OPSTR;                                            // string follows
                ...
                assert(sizeof(us) == sizeof(u_short));
                memcpy((*comp), &us, sizeof(u_short));                         // store short int
                (*comp) += sizeof(u_short);                                    // move past it
                *(*comp)++ = '^';                                              // store the caret
                for (j = 0; j < i; *(*comp)++ = rou.var_cu[j++]) continue;     // copy rou
                *(*comp)++ = '\0';                                             // and null terminate
                if (!var_empty(tag) || gotplus || p1indirect) *(*comp)++ = OPCAT; // concatenate
            }
...
exit:
    ...
    // Below if we are doing a DO sans indirection... we must copy tag and rou to (*comp)
    if (!var_empty(tag) || offset) {                                            // if we have a tag
        ...
        if ((!var_empty(rou)) || offset) {                                      // if we have a routine
            ...
            memcpy((*comp), &rou.var_qu, VAR_LEN);                             // save the routine
            (*comp) += VAR_LEN;
        }

        assert(sizeof(tag.var_qu) == VAR_LEN);
        memcpy((*comp), &tag.var_qu, VAR_LEN);                                 // save the tag
        (*comp) += VAR_LEN;
        ...
        if (offset) {                                                           // if we have an offset
            ...
            us = (u_short) offset;
            assert(sizeof(us) == sizeof(u_short));
            memcpy((*comp), &us, sizeof(u_short));                             // save the offset
            (*comp) += sizeof(u_short);
            return -4;                                                          // and exit
        }
 ...
 */
int Compile_Routine_temp(mvar *rou, mvar *src, u_char *stack)
{
