...
mod test_job_pool {
    ...
    fn no_open_slots() {
        ...
        let target_pid = ProcessID(789);
        let slot = find_open_slot(&mut job_table, target_pid, StartType::Run);
        assert!(slot.is_none());
        let slot = find_open_slot(&mut job_table, target_pid, StartType::Job);
        assert!(slot.is_none());
    }
    ...
    fn run_jobs_must_use_an_unoccupied_slot() {
        let mut job_table = [ZEROED_JOB_TAB; 2];
        let target_pid = ProcessID(123);

        // Mocking that the job_tabs are already in use.
        job_table[0].pid = target_pid.0;
        job_table[1].pid = 0;
        // Dirtying some field.
