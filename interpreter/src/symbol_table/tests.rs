...
use super::Table;
use crate::symbol_table::m_var::helpers::{var_m, var_m_non_null};
...
fn get_unset_key() {
    ...
    table.set(m_var.borrow(), &mut data).unwrap();

    let m_var = var_m_non_null("foo", &["bar"]);
    assert_eq!(table.get(&m_var), None);
}
...
fn set_index_value() {
    let mut table = Table::new();
    let m_var = var_m_non_null("foo", &["keys"]);
    let mut data = "Data".try_into().unwrap();
    table.set(&m_var, &mut data).unwrap();
...
fn set_root_then_index() {
    let root = var_m_non_null("foo", &[]);
    let root_data = "root Data".try_into().unwrap();
    let with_key = var_m_non_null("foo", &["keys"]);
...
fn set_works_while_with_prefixs() {
    let mut table = Table::new();
    let prefix = var_m_non_null("foo", &["prefix"]);
    let full = var_m_non_null("foo", &["prefixAndMore"]);
...
fn do_a_bunch_of_sets() {
    ...
    let test_data =
        test_data.map(|(keys, value)| (var_m_non_null("foo", &keys), value.try_into().unwrap()));
...
fn kill_uninitialized_var() {
    ...
    table.kill(&var_m_non_null("foo", &[]));
    table.kill(&var_m_non_null("foo", &["arg"]));
}

#[test]
fn kill_initialized_root() {
    let mut table = Table::new();
    let var = var_m_non_null("foo", &[]);
    let var_i = var_m_non_null("foo", &["i"]);
    let var_ii = var_m_non_null("foo", &["i", "ii"]);
...
fn kill_removes_only_specified_index() {
    let mut table = Table::new();
    let data = "data".try_into().unwrap();
    let a = var_m_non_null("foo", &["a"]);
    let b = var_m_non_null("foo", &["b"]);
    let c = var_m_non_null("foo", &["c"]);
...
fn keep_vars() {
    let mut table = Table::new();
    let data = "data".try_into().unwrap();
    let dolor = var_m_non_null("$dolor", &[]);
    let normal = var_m_non_null("normal", &[]);
    let retain_a = var_m_non_null("retain_a", &[]);
    let retain_b = var_m_non_null("retain_b", &[]);
