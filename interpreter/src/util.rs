...
pub fn info(file: String) {
    println!("{}", rsm_version());
    println!(
        "Database Version: {}\tCompiler Version: {}",
        DB_VER, COMP_VER
    );
    println!();
...
fn systab_info(file: String) -> Result<String, String> {
    ...
    let temp = unsafe { vol.rbd_end.offset_from(vol.rbd_head) } / 1048576;
    println!("Routine Buffer Space:\t{}\tMiB", temp);
    let temp = unsafe { *systab }.locksize / 1024;
    println!("Lock Table Size:\t{}\tKiB", temp);
