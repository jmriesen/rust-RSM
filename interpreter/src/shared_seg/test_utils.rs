...
pub struct Diff(BTreeMap<usize, (u8, u8)>);

impl Diff {
    pub fn new() -> Self {
        Self::default()
    }
    pub fn insert(&mut self, left: i32, right: i32, index: usize) {
...
pub fn test_memory_segment_equality(left: &[u8], right: &[u8]) -> Diff {
    ...
    while let Some((index, (left, right))) = iter.next() {
        //Try treating the data as a pointer and calculate their offsets from the baseline
