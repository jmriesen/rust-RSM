...
void dodollar_temp(u_char **src,u_char **comp,partab_struct *partab_ptr)                                                             // parse var, func etc.
{
    ...
    case 'T':                                                                   // $T[EST]
        ...
        *(*comp)++ = VART;                                                     // add the opcode
        return;                                                                 // and exit

    case 'X':                                                                   // $X
        if (len > 1) UNVAR;                                                     // check for extended name
        *(*comp)++ = VARX;                                                     // add the opcode
        return;                                                                 // and exit

    case 'Y':                                                                   // $Y
        if (len > 1) UNVAR;                                                     // check for extended name
        *(*comp)++ = VARY;                                                     // add the opcode
        return;                                                                 // and exit
